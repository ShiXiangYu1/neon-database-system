    ...
    try {
        ...
        try {
            ...
            if (!response || !response.sales || !response.orders) {
                console.warn("API返回的销售概览数据结构不完整，将使用模拟数据");
            ...
            console.log("已生成销售概览模拟数据:", response);
        }
        ...
        try {
            ...
            // 更新销售统计卡片 (使用应急数据)
            setElementText('total-sales', formatCurrency(fallbackData.sales.total));
            setElementText('today-sales', formatCurrency(fallbackData.sales.today));
            setElementText('yesterday-sales', formatCurrency(fallbackData.sales.yesterday));
            setElementText('week-sales', formatCurrency(fallbackData.sales.week));
            
            // 更新订单统计卡片 (使用应急数据)
            setElementText('total-orders', formatNumber(fallbackData.orders.total));
            setElementText('today-orders', formatNumber(fallbackData.orders.today));
            setElementText('pending-orders', formatNumber(fallbackData.orders.pending));
            setElementText('month-orders', formatNumber(fallbackData.orders.month));
            
            console.log("已使用应急数据显示销售概览");
        } catch (fallbackError) {
            console.error("应急数据显示也失败:", fallbackError);
